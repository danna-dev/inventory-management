...
export class ProductService {
  ...
  async findAll( productFiltersDto : ProductFiltersDto ): Promise<Product[]> {
    ...
    if( filter.minPrice ) queryBuilder.andWhere('product.price >= :minPrice', { minPrice: filter.minPrice })

    if( filter.maxPrice ) queryBuilder.andWhere('product.price <= :maxPrice', { maxPrice: filter.maxPrice })
